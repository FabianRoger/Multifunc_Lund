...
pca_mf %>% 
  ...
  lapply(function(x) {
    x %>% 
      dplyr::summarise(`PCA MF cor` = cor(`average MF`, `PCA MF`),
                   `Pasari MF cor` = cor(`average MF`, `Pasari MF`),
                   var_cor_mean = mean(cor(.[,func.names])))}) %>% 
      bind_rows() %>% 
  mutate(row = 1:n()) %>% 
  pivot_longer(!one_of("row", "var_cor_mean"), names_to = "metric", values_to = "value") %>%
  ggplot(aes(x = var_cor_mean, y = value)) +
  geom_point()+
  geom_hline(yintercept = 0)+
  facet_wrap(~metric) +
  theme_minimal()+
  labs(y = "correlation with average MF", x = "average Function correlation")
```
